...
impl Delivery {
	pub fn new(delivery: message::Delivery, channel: mpsc::Sender<Completion>) -> Result<Self, Error> {
		...
		let properties = {
			...
			props.root_id = match headers.get("root_id") {
				Some(AMQPValue::LongString(raw_id)) => Some(Uuid::parse_str(&raw_id).map_err(Error::from)?),
	...
	pub fn ack(self) -> Acknowledge {
		let task = self.channel.send(Completion::Acknowledge(self.delivery_tag))
			.map(|_| ())
			.map_err(Error::from);
	...
	pub fn reject(self) -> Reject {
		let task = self.channel.send(Completion::Reject(self.delivery_tag))
			.map(|_| ())
			.map_err(Error::from);
		Reject(Box::new(task))
	}
