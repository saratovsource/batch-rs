...
use lapin::channel::{BasicConsumeOptions, BasicPublishOptions, Channel, ExchangeDeclareOptions, QueueBindOptions, QueueDeclareOptions};
            ...
            .and_then(move |(client, mut heartbeat)| {
                ...
                client.create_channel()
                    ...
                    .map(|channel| {
                        ...
                        spawn(background);
                        Connection { inner: Inner { _handle: handle, client, channel, publisher, queues: BTreeMap::new() } }
                    })
            })
    }
}
...
impl batch::Declarator<exchange::Builder, exchange::Exchange> for Connection {
    ...
    fn declare(&mut self, builder: exchange::Builder) -> Self::DeclareFuture {
        ...
        let task = self.inner.channel.exchange_declare(
                &builder.name,
                builder.kind.as_ref(),
...
impl batch::Declarator<queue::Builder, queue::Queue> for Connection {
    ...
    fn declare(&mut self, builder: queue::Builder) -> Self::DeclareFuture {
        ...
        self.inner.queues.insert(queue.name().into(), queue.bindings.clone());
...
impl batch::ToConsumer for Connection {
    ...
    fn to_consumer(&mut self, queues: impl IntoIterator<Item = impl AsRef<str>>) -> Self::ToConsumerFuture {
        let names: HashSet<String> = HashSet::from_iter(queues.into_iter().map(|q| q.as_ref().to_string()));
        let names: Vec<String> = self.inner.queues.keys()
            .filter(|q| names.contains(q.as_str()))
            .map(ToString::to_string)
            .collect();
        let queues = self.inner.queues.clone();
        let task = self.inner.client.create_channel()
            .map_err(Error::from)
            .and_then(move |channel| {
                let tasks: Vec<Box<Future<Item = Queue, Error = Error> + Send>> = names.iter().map(|queue_name| {
                    let task = channel.queue_declare(&queue_name, QueueDeclareOptions::default(), FieldTable::new())
                        .map_err(Error::from);
                    let boxed: Box<Future<Item = Queue, Error = Error> + Send> = Box::new(task);
                    boxed
                }).collect();
                future::join_all(tasks)
                    .map(|names| (names, channel))
            })
            .and_then(move |(names, channel)| {
                ...
                let tasks: Vec<Box<Future<Item = (), Error = Error> + Send>> = bindings.into_iter().map(|(queue, binding)| {
                    let task = channel.queue_bind(
                        &queue,
                        &binding.exchange,
                        &binding.routing_key,
                    ...
                    let boxed: Box<Future<Item = (), Error = Error> + Send> = Box::new(task);
                    boxed
                }).collect();
                future::join_all(tasks)
                    .map(move |_| (names, channel))
            })
            .and_then(|(names, channel)| {
                let tasks: Vec<Box<Future<Item = consumer::Consumer<stream::Stream>, Error = Error> + Send>> = names.iter().map(|queue_name| {
                    let task = channel.basic_consume(&queue_name, "", BasicConsumeOptions::default(), FieldTable::new())
                        .map_err(Error::from);
                    let boxed: Box<Future<Item = consumer::Consumer<stream::Stream>, Error = Error> + Send> = Box::new(task);
                    boxed
                }).collect();
                future::join_all(tasks)
                    .map(|consumers| (consumers, channel))
            })
            .and_then(|(mut consumers, channel)| {
                let combined: Box<Stream<Item = message::Delivery, Error = Error> + Send + 'static> =
