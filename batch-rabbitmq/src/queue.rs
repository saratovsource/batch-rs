...
pub struct Builder {
	...
	pub(crate) callbacks: BTreeMap<String, fn(&[u8], batch::Container) -> Box<Future<Item = (), Error = Error> + Send>>,
}
...
impl Builder {
	...
	where
		...
		J: batch::Job,
	{
		let callback = |payload: &[u8], context: batch::Container| -> Box<Future<Item = (), Error = Error> + Send> {
	...
	pub fn declare(self, declarator: &mut impl batch::Declarator<Builder, Queue>) -> impl Future<Item = Queue, Error = Error> {
		declarator.declare(self)
	}

	pub(crate) fn build(self) -> Result<Queue, Error> {
		Ok(Queue { name: self.name, bindings: self.bindings, callbacks: self.callbacks })
	}
}
...
impl batch::Callbacks for Queue {
	type Iterator = IntoIter<String, fn(&[u8], batch::Container) -> Box<Future<Item = (), Error = Error> + Send>>;
