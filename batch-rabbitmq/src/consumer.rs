...
impl Consumer {
	pub(crate) fn new(channel: channel::Channel<stream::Stream>, inner: Box<Stream<Item = message::Delivery, Error = Error> + Send + 'static>) -> Self {
		let (tx, rx) = mpsc::channel(512);
		let rx = rx.for_each(move |completion| -> Box<Future<Item = (), Error = ()> + Send> {
			match completion {
				Completion::Acknowledge(delivery_tag) => {
					let task = channel.basic_ack(delivery_tag, false)
						.map_err(|e| error!("An error occured while acknowledging a delivery: {}", e));
					Box::new(task)
				}
				Completion::Reject(delivery_tag) => {
					...
					// execution.
					let task = channel.basic_reject(delivery_tag, true)
						.map_err(|e| error!("An error occured while rejecting a delivery: {}", e));
					Box::new(task)
				}
			}
		});
