...
impl Dispatch {
	pub(crate) fn new(exchange: String, job: impl batch::Job, properties: batch::Properties) -> Result<Self, Error> {
		Ok(Dispatch {
	...
	pub fn to_amqp_properties(&self) -> BasicProperties {
		...
		headers.insert("task".into(), AMQPValue::LongString(self.properties.task.clone()));
