...
pub trait Declare: Sized {
    ...
    fn declare(declarator: &mut (impl Declarator<Self::Input, Self::Output> + Send + 'static)) -> Self::DeclareFuture;
}
...
pub trait Callbacks {
    ...
    type Iterator: Iterator<Item = (String, fn(&[u8], Container) -> Box<Future<Item = (), Error = Error> + Send>)>;
    ...
    /// * `container`: A type-map containing the types managed by the executor (usually some kind `Worker`).
    fn callbacks(&self) -> Self::Iterator;

    // /// TODO: dox
    // fn callback(&self, key: &str, delivery: &Self::Delivery, context: &Container) -> Result<(), Error>;
}
